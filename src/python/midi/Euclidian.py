...
def _densifier(start_pattern_num, pattern_length):
    ...
    for target in range(start_trigger_count+1, pattern_length):
...
def densifier(pat):
    ...
    for np in num_patterns:
        ret.append(number_to_pattern(np))
    return ret


if __name__ == "__main__":
    for p in get_euclidian_patterns(15):
        print(number_to_pattern(p))
    print()
    for i in range(0,16):
        print(number_to_pattern(i), pattern_to_number(number_to_pattern(i)))
    print()
    pat = "x   x   x   xx x"
    print(pat.upper())
    print('\n'.join(densifier(pat)))